...
import { Image, Pressable, Text, TextInput, View, Platform } from 'react-native';
...
import { swapCurrencies, setFromCurrency } from '../../redux/slices/settingsSlice';
...
export default function ScanPreviewScreen() {
  ...
  const { uri, candidates } = route.params;
  ...
  const onPickCandidate = (c: Candidate) => {
    // put candidate value into mini converter; if candidate has its own currency, use it
  ...
  return (
    <View style={styles.root}>
      ...
      <BottomSheet ref={sheetRef} index={1} snapPoints={snaps} handleIndicatorStyle={styles.handle}>
        <BottomSheetScrollView contentContainerStyle={styles.sheetContent} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.miniRow}>
            <View style={styles.miniTop}>
              ...
              <Pressable onPress={() => dispatch(swapCurrencies())} style={styles.swapBtn}>
            ...
            <View style={styles.miniInputs}>
              ...
              <View style={styles.toBox}>
                ...
                <Text style={styles.miniConverted}>
                  {baseRate ? new Intl.NumberFormat(undefined, { minimumFractionDigits: decimals, maximumFractionDigits: decimals }).format(miniConverted) : '—'}
                </Text>
              </View>
            </View>

            <Text style={styles.rateNote}>
              Rate: {baseRate ? new Intl.NumberFormat(undefined, { maximumFractionDigits: 4 }).format(baseRate) : '—'} {to}
            </Text>
          </View>
...
{candidates?.length ? candidates.map((c, i) => (
  <Pressable key={`${c.value}-${c.currency ?? 'UNK'}-${i}`} onPress={() => onPickCandidate(c)} style={styles.row}>
    <View style={{flex:1}}>
      ...
      <Text style={styles.itemSub}>
        ...
        {c.currency ? `${c.currency} ` : ''}{c.value.toFixed(2)}
      </Text>
    </View>

    <ConvertedPill
      amount={c.value}
      fromCurrency={c.currency ?? from}   // if OCR found a currency, use it; else fallback
      toCurrency={to}
      decimals={decimals}
    />
  </Pressable>
...
/** Small pill that fetches the rate for (fromCurrency -> toCurrency) and prints converted */
function ConvertedPill({
  amount, fromCurrency, toCurrency, decimals,
}: { amount:number; fromCurrency:string; toCurrency:string; decimals:number }) {
  const samePair = useSelector((s:RootState)=>s.settings.fromCurrency) === fromCurrency &&
                   useSelector((s:RootState)=>s.settings.toCurrency) === toCurrency;

  // If same as global pair, reuse that query through a duplicate call (RTKQ will de-dupe)
  const { data } = useGetPairRateQuery({ from: fromCurrency, to: toCurrency });
  ...
  return (
    <View style={styles.convPill}>
      <Text style={styles.convMain}>
        {rate ? new Intl.NumberFormat(undefined, { minimumFractionDigits: decimals, maximumFractionDigits: decimals }).format(converted) : '…'}
      </Text>
