...
function detectPriceCandidatesWithLabels(lines: string[]): Candidate[] {
  ...
  const re =
    /(?:([€$£¥₩₺₪₹])|\b([A-Z]{3})\b|\blei\.?\b)?\s*-?\s*(\d[\d.,]*)\s*(?:([€$£¥₩₺₪₹])|\b([A-Z]{3})\b|\blei\.?\b)?/gi;

  norm.forEach((line, i) => {
    if (!line) return;

    for (const m of line.matchAll(re) as Iterable<
    ...
    >) {
      ...
      if (!label || label.length < 2) {
        for (let j = i - 1; j >= 0; j--) {
          ...
          if (prev && !/\d[\d.,]*\s*$/.test(prev)) {
            // previous line that isn't just a number
...
export async function pickImageAndDetectPrices(
  limit = 8,
): Promise<{ candidates: Candidate[]; asset?: Asset } | null> {
